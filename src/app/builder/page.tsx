...
import { Save, Eye, Globe, ArrowLeft, Layout, Cpu, MessageSquare, Sparkles, Monitor, Tablet, Smartphone, Download, Code } from 'lucide-react';
...
export default function BuilderPage() {
  const { mode, setMode, blocks, reset, viewport, setViewport, botToken } = useBuilderStore();
...
function ModeCard({ icon: Icon, title, desc, onClick, color, bgColor }: any) {
  return (
    <button onClick={onClick} className="group p-8 bg-card border rounded-3xl text-left hover:border-primary/50 transition-all flex flex-col items-start gap-4">
