...
import { Shield, Mail, Globe, CheckCircle2, Terminal, Scale, EyeOff, Clock, Info } from "lucide-react";
...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#020617] text-slate-50 font-body overflow-x-hidden flex flex-col">
      ...
      <header className="border-b border-white/5 bg-[#020617]/50 backdrop-blur-xl shrink-0 sticky top-0 z-50">
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-6">
            <Badge variant="outline" className="hidden sm:flex border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-[10px] font-bold tracking-[0.2em] px-3 py-0.5 rounded-full">
              V1.0.2 STABLE
            </Badge>
            <Link href="/admin">
      ...
      <main className="flex-1 container mx-auto px-6 py-8 grid lg:grid-cols-12 gap-8 items-center max-h-[calc(100vh-8rem)]">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              ...
              <Card key={i} className="bg-white/[0.02] border-white/5 shadow-none p-4 space-y-2 hover:bg-white/[0.04] transition-colors border-t-white/10">
                <div className={`bg-${item.color}-500/10 w-8 h-8 rounded-lg flex items-center justify-center`}>
                  <item.icon className={`w-4 h-4 text-${item.color}-400`} />
                </div>
                <h3 className="text-xs font-bold tracking-normal">{item.title}</h3>
                <p className="text-[11px] text-slate-400 leading-normal">{item.desc}</p>
              </Card>
            ))}
          </div>

          <div className="p-4 bg-gradient-to-r from-primary/10 to-transparent rounded-2xl border-l-2 border-primary shadow-inner">
            <p className="text-xs text-slate-400 leading-relaxed flex items-start gap-3">
              <Info className="w-4 h-4 text-primary shrink-0 mt-0.5" />
              <span>This page serves as the authoritative source for Cloudflare Bot Verification. We maintain 100% transparency in our scanning protocols.</span>
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="bg-white/[0.03] border-white/10 backdrop-blur-md shadow-2xl border-t-white/20 overflow-hidden">
            ...
            <CardContent className="space-y-4 pt-6">
              <div className="space-y-2">
                ...
                <div className="p-3 bg-black/40 rounded-lg border border-white/5 font-mono text-[10px] text-slate-300 break-all leading-relaxed shadow-inner">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-white/5 rounded-lg border border-white/5 space-y-1">
                  ...
                  <div className="text-[10px] font-bold">HTTP/2, TLS 1.3</div>
                </div>
                <div className="p-3 bg-white/5 rounded-lg border border-white/5 space-y-1">
                  <label className="text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold">Standard</label>
                  <div className="text-[10px] font-bold">RFC 9309 Compliance</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-indigo-500/10 to-primary/10 border-primary/20 backdrop-blur-md p-6">
            ...
            <p className="text-[11px] text-slate-400 leading-normal mb-4">
              To exclude your domain, update <code className="text-primary font-mono font-bold">robots.txt</code> or email our abuse desk.
            </p>
      ...
      <footer className="py-4 px-6 border-t border-white/5 bg-[#010413]/50 mt-auto">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-[9px] text-slate-500 uppercase tracking-[0.3em] font-bold">
